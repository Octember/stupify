...
> diff against them. Taste can't be auto-extracted: hand-pick these, and say _why_ each is good. A vague
...
- Keep a short code excerpt that captures the pattern. The point is the _principle_, not the lines.
...
the orchestrator only _coordinates_; every concern is a small focused unit, and every operation is the same
