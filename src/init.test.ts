...
function initIn(cwd: string, sub: string[]): CliResult {
  return spawnSync('bun', [CLI, 'init', ...sub], { cwd, encoding: 'utf8' })
}
...
test('init writes repo-root-relative paths + inlines real code, even run from a subdir', () => {
  ...
  expectSuccess(initIn(join(root, 'src/deep'), ['../a.ts'])) // from a subdir, with a cwd-relative path
  const c = corpus(root)
...
test('init refuses to overwrite an existing CORPUS without --force', () => {
  ...
  expectSuccess(initIn(root, ['src/b.ts'])) // no --force
